...
def index(request):
	context = {
		...
		'teams_in_atlantic_soccer':Team.objects.filter(league__name="Atlantic Soccer Conference"), # all teams in the Atlantic Soccer Conference
		'players_on_boston':Player.objects.filter(curr_team__location="Boston", curr_team__team_name="Penguins"), # all (current) players on the Boston Penguins
